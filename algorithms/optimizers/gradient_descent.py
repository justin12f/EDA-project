...
class VerifyConvergence:
    ...
    def verify_convergence(
        self, loss_history: list[float], tolerance: float, iteration: int
    ) -> bool:
...
class GradientDescentOptimizer:
    ...
    def optimize(
        ...
        for iteration in range(max_iterations):
            ...
            if VerifyConvergence().verify_convergence(
                loss_history, tolerance, iteration
            ):
                converged_ = True
                break
...
class GradientDescent:
    ...
    def optimize(
        ...
        # Force column vectors if necessary
        if x.ndim == 1:
            x = x.reshape(-1, 1)
        if y.ndim == 1:
            y = y.reshape(-1, 1)
