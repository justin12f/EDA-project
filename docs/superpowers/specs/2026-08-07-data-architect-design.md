...
| # | Decision | Rationale |
|---|---|---|
| D1 | **Postgres, schema-per-org in the existing Supabase instance**, with **one Postgres role per org** | Zero new infrastructure; the session pooler is already solved; real concurrency; and FKs are genuinely enforced, so the diagram is *read* from `information_schema` rather than guessed |
...
This is the security crux: **an agent writes the DDL**, and `organizations`, `subscriptions`, and `api_keys` live in the same database.

At org bootstrap, three objects are created:
...
**The two-session rule.** Because `tenant_<hex>_role` has no grant on `public`, a tenant session **cannot** write `proposals`, `artifact_dependencies`, `drift_events`, or any other control-plane row. Every operation touching both therefore uses two sessions in sequence, never one:
...
### 3.3 DDL rendering — the one injection surface
...
### 3.4 Source adapters — "any format"
...
| `services/api/src/lumen_api/tenant_db.py` | schema/role provisioning, `tenant_session()` | `db/session.py` |
...
**`HandleStore.resolve()` changes substrate.** It stops downloading Parquet and starts issuing `pl.read_database()` against the tenant schema. Because the engine's contract is a DataFrame, ADR-0008 through ADR-0013 need no changes. Parquet remains as a compute cache and an implementation detail.
...
**Blast radius.** The per-org role (§3.1) means a compromised or buggy Architect cannot reach `public` or another tenant. This is the single most important security property of the design and gets a dedicated integration test (§8).
...
1. **Tenant isolation — the critical test.** Under `tenant_A_role`: `SELECT` against `tenant_B.*` must fail, and `SELECT` against `public.organizations` must fail. If this test does not exist, the design is not implemented.
...
- **Cross-database foreign keys.** If a customer connects two databases, can an FK span them? Postgres cannot enforce a constraint across a schema boundary it does not own, so such a relationship can only ever be `observed`. Not blocking; resolve when a second DB source exists.
- **Storage cost attribution.** Tenant schemas consume Supabase storage that no plan limit currently meters. `compute_seconds` exists as an ungated metric with no plan-limit column; a `storage_bytes` metric belongs to the same gap and to ADR-0004's territory, not this one.
- **Very large tables.** `pl.read_database()` materialises fully, unlike `scan_parquet`'s lazy pushdown, which `materialize.py`'s own docstring cites as a design reason. At the dataset sizes this product handles today this is acceptable; the threshold at which it stops being acceptable is unmeasured.
