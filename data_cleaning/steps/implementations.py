...
def detect_bool_columns(df: pd.DataFrame) -> list[str]:
    ...
    for columns in df.columns:
        if (
            pd.api.types.is_string_dtype(df[columns])
            or pd.api.types.is_object_dtype(df[columns])
            or pd.api.types.is_bool_dtype(df[columns])
        ):
            unique_vals = (
                df[columns].dropna().astype(str).str.lower().str.strip().unique()
            )
            val_set = set(unique_vals) - {"nan", "<na>", "none", ""}
            if len(val_set) > 0 and val_set.issubset(bool_vocab):
...
def detect_numeric_columns(df: pd.DataFrame) -> list[str]:
    ...
    for col in df.select_dtypes(include="object").columns:
        if infer_column_type(df[col]) == "numeric":
            num_cols.append(col)
    return [c for c in num_cols if not c.endswith("_id") and c != "id"]
...
class FixBoolsColumnsStep(BaseStep):
    ...
    def process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in cols:
            mapped = df[col].astype(str).str.lower().str.strip().map(self.BOOL_MAP)
